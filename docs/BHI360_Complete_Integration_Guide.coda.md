...
```mermaid
graph TD
    A[Application<br/>motion_sensor.cpp] --> B[BHI360 Driver<br/>bhi360_zephyr.c]
    B --> C[Calibration Module<br/>bhi360_calibration]
    C --> D[BHY2 API<br/>Bosch Library]
    D --> E[Hardware SPI<br/>BHI360 Chip]
```
...
graph TD
    ...
    B -->|Status >= Threshold| C[Skip Calibration]
    B -->|Status < Threshold| D[Gyro Calibration<br/>Device Stationary]
    D --> E[Accel Calibration<br/>Device Flat, Z-up]
    E --> F[Update Status]
    F --> G[Continue]
    C --> G
```
