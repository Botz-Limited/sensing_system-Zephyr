...
static constexpr uint32_t DISCOVERY_INITIAL_DELAY_MS =
    10; // Delay before starting discovery
...
void send_device_info_to_primary(void) {
  ...
  strncpy(msg.data.device_info.manufacturer, dis_get_manufacturer(),
          sizeof(msg.data.device_info.manufacturer) - 1);
  strncpy(msg.data.device_info.model, dis_get_model(),
          sizeof(msg.data.device_info.model) - 1);
  strncpy(msg.data.device_info.serial, dis_get_serial(),
          sizeof(msg.data.device_info.serial) - 1);
  strncpy(msg.data.device_info.hw_rev, dis_get_hw_revision(),
          sizeof(msg.data.device_info.hw_rev) - 1);
  strncpy(msg.data.device_info.fw_rev, dis_get_fw_revision(),
          sizeof(msg.data.device_info.fw_rev) - 1);

  // Ensure null termination
  msg.data.device_info
      .manufacturer[sizeof(msg.data.device_info.manufacturer) - 1] = '\0';
...
static const uint8_t target_service_uuid[16] = {
    0xcd, 0xc8, 0x9e, 0xfc, 0x1f, 0x00, 0xa8, 0x89,
...
static void device_found(const bt_addr_le_t *addr, int8_t rssi, uint8_t type,
  ...
  char addr_str[BT_ADDR_LE_STR_LEN];
  bt_addr_le_to_str(addr, addr_str, sizeof(addr_str));
  LOG_INF("[SCAN] Device found: %s, RSSI: %d, type: %u, adv data len: %u",
          addr_str, rssi, type, ad ? ad->len : 0);
...
K_WORK_DELAYABLE_DEFINE(weight_measurement_timeout_work,
                        weight_measurement_timeout_work_handler);
...
static const struct bt_data sd[] = {
    BT_DATA_BYTES(
        BT_DATA_UUID128_ALL,
        BT_UUID_128_ENCODE(0x5cb36a14, 0xca69, 0x4d97, 0x89a8, 0x001ffc9ec8cd)),
};
...
err_t bt_module_init(void) {
  ...
  if (conn_params_err != 0) {
    LOG_ERR("Failed to initialize connection parameter manager: %d",
            conn_params_err);
  } else {
  ...
  fota_progress_msg_t clean_progress = {.is_active = false,
                                        .status = 0,
                                        .percent_complete = 0,
                                        .bytes_received = 0,
                                        .total_size = 0,
...
// MCUmgr SMP Bluetooth transport is automatically registered when enabled
  ...
  int adv_err = bt_le_adv_start(BT_LE_ADV_CONN_NAME, ad, ARRAY_SIZE(ad), sd,
                                ARRAY_SIZE(sd));
  if (adv_err) {
  ...
  bluetooth_tid = k_thread_create(&bluetooth_thread_data, bluetooth_stack_area,
                                  K_THREAD_STACK_SIZEOF(bluetooth_stack_area),
                                  bluetooth_process, nullptr, nullptr, nullptr,
                                  bluetooth_priority, 0, K_NO_WAIT);
...
void bluetooth_process(void * /*unused*/, void * /*unused*/,
                       void * /*unused*/) {
  ...
  while (true) {
    ...
    if (ret == 0) { // Message successfully received
      ...
      case MSG_TYPE_FOOT_SAMPLES: {
        ...
        foot_samples_t *foot_data =
            &msg.data.foot_samples; // Get pointer to data within the message
#if IS_ENABLED(CONFIG_PRIMARY_DEVICE)
      ...
      case MSG_TYPE_BHI360_3D_MAPPING: {
        ...
        LOG_INF("Received BHI360 3D Mapping from %s: Accel(%.2f,%.2f,%.2f), "
                "Gyro(%.2f,%.2f,%.2f)",
                get_sender_name(msg.sender), (double)mapping_data->accel_x,
                (double)mapping_data->accel_y, (double)mapping_data->accel_z,
                (double)mapping_data->gyro_x, (double)mapping_data->gyro_y,
        ...
        if (msg.sender == SENDER_D2D_SECONDARY) {
          ...
          LOG_WRN(
              "Secondary BHI360 3D data received - needs separate handling!");
      ...
      case MSG_TYPE_BHI360_LINEAR_ACCEL: {
        ...
        LOG_INF("Received BHI360 Linear Accel from %s: (%.2f,%.2f,%.2f)",
                get_sender_name(msg.sender), (double)lacc_data->x,
                (double)lacc_data->y, (double)lacc_data->z);
#if IS_ENABLED(CONFIG_PRIMARY_DEVICE)
        // Primary device: Check sender to avoid mixing primary/secondary data
        if (msg.sender == SENDER_D2D_SECONDARY) {
          // Secondary BHI360 linear accel data - needs separate handling
          LOG_WRN("Secondary BHI360 linear accel received - needs separate "
                  "handling!");
      ...
      case MSG_TYPE_BHI360_STEP_COUNT: {
        ...
        if (msg.sender == SENDER_MOTION_SENSOR) {
          ...
          uint32_t total_activity_steps =
              primary_activity_steps + secondary_activity_steps;
...
#else
        LOG_INF("Secondary device sending step count: %u",
      ...
      case MSG_TYPE_ACTIVITY_STEP_COUNT: {
        ...
        if (msg.sender == SENDER_D2D_SECONDARY) {
          ...
          uint32_t total_activity_steps =
              primary_activity_steps + secondary_activity_steps;
          LOG_INF(
              "Secondary Activity Step Count Update - Secondary=%u, Total=%u",
      ...
      case MSG_TYPE_COMMAND: {
        ...
        LOG_INF("Received Command from %s: '%s'", get_sender_name(msg.sender),
                command_str);
      ...
      case MSG_TYPE_FOTA_PROGRESS: {
        ...
        if (!progress->is_active && progress->bytes_received > 0) {
          LOG_WRN(
              "Stale FOTA progress detected: bytes=%u, percent=%d%% (ignoring)",
        ...
        LOG_INF("Received FOTA Progress: active=%d, status=%d, percent=%d%%, "
                "bytes=%u/%u",
                progress->is_active, progress->status,
                progress->percent_complete, progress->bytes_received,
                progress->total_size);
        ...
        if (progress->status == 2 && progress->bytes_received > 500000 &&
            !app_core_done) {
          LOG_INF("===========================================");
        ...
        if (progress->status == 2 && progress->bytes_received < 200000 &&
            progress->bytes_received > 100000) {
          LOG_INF("===========================================");
        ...
        if (progress->is_active && progress->status == 1 && !fota_was_active) {
          ...
          if (active_conn) {
            ...
            if (info_err == 0 && info.state == BT_CONN_STATE_CONNECTED) {
              LOG_INF("Updating connection parameters for FOTA - switching to "
                      "BACKGROUND profile");
              int conn_err =
                  ble_conn_params_update(active_conn, CONN_PROFILE_BACKGROUND);
              if (conn_err) {
                if (conn_err == -EINVAL) {
                  ...
                  conn_err = ble_conn_params_update(active_conn,
          ...
          strncpy(fota_msg.data.command_str, "FOTA_IN_PROGRESS",
                  sizeof(fota_msg.data.command_str) - 1);
          ...
          if (d2d_conn != nullptr) {
            LOG_INF(
                "Sending stop activity command to secondary device for FOTA");
            ...
            if (d2d_err != 0) {
              ...
              d2d_err = ble_d2d_tx_queue_command(D2D_TX_CMD_STOP_ACTIVITY,
                                                 &stop_value);
              if (d2d_err) {
        ...
        if (fota_was_active && (!progress->is_active || progress->status == 3 ||
                                progress->status == 4)) {
          LOG_INF(
              "FOTA update complete/failed - resuming normal BLE operation");
        ...
        if (!progress->is_active && progress->status == 0 &&
      ...
      case MSG_TYPE_ERROR_STATUS: {
        ...
        LOG_INF("Received Error Status from %s: error_code=%d, is_set=%s",
                get_sender_name(msg.sender), (int)error_status->error_code,
      ...
      case MSG_TYPE_NEW_ACTIVITY_LOG_FILE: {
        ...
        LOG_INF("Received NEW ACTIVITY LOG FILE notification from %s:",
        ...
        // Secondary device: Send to primary via D2D
        int err =
            d2d_tx_notify_activity_log_available(log_info->file_sequence_id);
        if (err) {
...
#if IS_ENABLED(CONFIG_PRIMARY_DEVICE)
        // Handle activity metrics update from realtime_metrics module (primary
        // only)
      ...
      case MSG_TYPE_DEVICE_INFO: {
        ...
        jis_update_secondary_device_info(dev_info->manufacturer,
                                         dev_info->model, dev_info->serial,
        ...
        // send_device_info_to_primary()
        LOG_DBG(
            "Device info from secondary is sent automatically on connection");
#endif
        break;
      }
      ...
      case MSG_TYPE_WEIGHT_MEASUREMENT: {
        ...
        if (msg.sender == SENDER_ACTIVITY_METRICS) {
          ...
          if (d2d_conn != nullptr) {
            ...
            LOG_INF(
                "Primary weight measured: %.1f kg, requesting secondary weight",
      ...
      default:
        LOG_WRN("Unknown message type received (%d) from %s", msg.type,
                get_sender_name(msg.sender));
        break;
      }
    }
...
err_t bt_start_advertising(int err) {
  ...
  LOG_INF("Starting advertising with params: options=0x%02x, interval_min=%d, "
          "interval_max=%d",
          bt_le_adv_conn_name.options, bt_le_adv_conn_name.interval_min,
  ...
  if (err) {
    ...
    if (err == -ENOENT || err == -EINVAL) {
      ...
      err = bt_le_adv_start(&temp_adv_params, ad, ARRAY_SIZE(ad), sd,
                            ARRAY_SIZE(sd));
      if (err) {
