...
static struct bt_gatt_subscribe_params subscribe_params[18] =
    {}; // Increased for new D2D batch char and future expansion
...
static uint8_t foot_sensor_notify_handler(struct bt_conn *conn, struct bt_gatt_subscribe_params *params,
                                          const void *data, uint16_t length);
static uint8_t bhi360_data1_notify_handler(struct bt_conn *conn, struct bt_gatt_subscribe_params *params,
                                           const void *data, uint16_t length);
static uint8_t bhi360_data2_notify_handler(struct bt_conn *conn, struct bt_gatt_subscribe_params *params,
                                           const void *data, uint16_t length);
static uint8_t bhi360_data3_notify_handler(struct bt_conn *conn, struct bt_gatt_subscribe_params *params,
                                           const void *data, uint16_t length);
static uint8_t status_notify_handler(struct bt_conn *conn, struct bt_gatt_subscribe_params *params, const void *data,
                                     uint16_t length);
static uint8_t charge_status_notify_handler(struct bt_conn *conn, struct bt_gatt_subscribe_params *params,
                                            const void *data, uint16_t length);
static uint8_t foot_log_notify_handler(struct bt_conn *conn, struct bt_gatt_subscribe_params *params, const void *data,
                                       uint16_t length);
static uint8_t bhi360_log_notify_handler(struct bt_conn *conn, struct bt_gatt_subscribe_params *params,
                                         const void *data, uint16_t length);
static uint8_t device_info_notify_handler(struct bt_conn *conn, struct bt_gatt_subscribe_params *params,
                                          const void *data, uint16_t length);
static uint8_t fota_progress_notify_handler(struct bt_conn *conn, struct bt_gatt_subscribe_params *params,
                                            const void *data, uint16_t length);
static uint8_t foot_log_path_notify_handler(struct bt_conn *conn, struct bt_gatt_subscribe_params *params,
                                            const void *data, uint16_t length);
static uint8_t bhi360_log_path_notify_handler(struct bt_conn *conn, struct bt_gatt_subscribe_params *params,
                                              const void *data, uint16_t length);
static uint8_t activity_log_available_notify_handler(struct bt_conn *conn, struct bt_gatt_subscribe_params *params,
                                                     const void *data, uint16_t length);
static uint8_t activity_log_path_notify_handler(struct bt_conn *conn, struct bt_gatt_subscribe_params *params,
                                                const void *data, uint16_t length);
...
static struct subscribe_work_data subscribe_work_items[18];
...
static void smp_discovery_work_handler(struct k_work *work)
{
    ...
    {
        LOG_WRN("Not all subscriptions completed yet (%d/%d), rescheduling SMP discovery", completed, pending);
...
static void perform_subscriptions(void)
{
    ...
    LOG_INF("Handles - foot:%u, bhi1:%u, bhi2:%u, bhi3:%u, status:%u, charge:%u", foot_sensor_handle,
            bhi360_data1_handle, bhi360_data2_handle, bhi360_data3_handle, status_handle, charge_status_handle);
    LOG_INF("         foot_log:%u, bhi_log:%u, dev_info:%u, fota:%u", foot_log_handle, bhi360_log_handle,
    ...
    // Add D2D batch subscription
    if (d2d_batch_handle && work_index < WORK_INDEX_MX)
    {
        k_work_init(&subscribe_work_items[work_index].work, individual_subscribe_work_handler);
                                           ...
                                           const void *data, uint16_t length)
{
    ...
    LOG_INF("  Gyro: X=%.4f, Y=%.4f, Z=%.4f rad/s", (double)mapping.gyro_x, (double)mapping.gyro_y,
            (double)mapping.gyro_z);
    LOG_INF("  Quaternion: X=%.4f, Y=%.4f, Z=%.4f, W=%.4f", (double)mapping.accel_x, (double)mapping.accel_y,
            (double)mapping.accel_z, (double)mapping.quat_w);
                                           ...
                                           const void *data, uint16_t length)
{
    ...
    LOG_INF("  Linear Accel: X=%.2f, Y=%.2f, Z=%.2f", (double)accel->x, (double)accel->y, (double)accel->z);
...
static void subscribe_to_characteristic(uint16_t handle, bt_gatt_notify_func_t notify_func, int index)
{
    ...
    while (retry_count > 0)
    {
        ...
        LOG_WRN("Failed to subscribe to handle %u at index %d: %d (retries left: %d)", handle, index, err,
                retry_count - 1);
...
// DIS read callback
static uint8_t dis_read_callback(struct bt_conn *conn, uint8_t err, struct bt_gatt_read_params *params,
                                 const void *data, uint16_t length)
{
    ARG_UNUSED(conn);
    ...
    if (dis_chars_read >= 5)
    {
        ...
        jis_update_secondary_device_info(dis_manufacturer, dis_model, dis_serial, dis_hw_rev, dis_fw_rev);
    }
                             ...
                             struct bt_gatt_discover_params *params)
{
    ...
    else if (discovery_state == DISCOVER_CHARACTERISTICS)
    {
        ...
        else if (bt_uuid_cmp(chrc->uuid, &d2d_batch_uuid.uuid) == 0)
        {
            d2d_batch_handle = chrc->value_handle;
                                        ...
                                        uint16_t length)
{
    ...
    if (!secondary_conn || conn != secondary_conn)
    {
        LOG_WRN("D2D batch notification after disconnection, ignoring");
        return BT_GATT_ITER_STOP;
    }
    ...
    if (!data)
    {
        LOG_WRN("D2D batch unsubscribed");
        params->value_handle = 0;
        return BT_GATT_ITER_STOP;
    }
