 ...
 *       Data Requirements: Motion sensor data (BHI360 for step count and cadence), Foot sensor data (both feet for
 * contact times and balance).
 */
...
static void realtime_metrics_init(void)
{
    ...
    k_work_queue_start(&realtime_metrics_work_q, realtime_metrics_workq_stack,
                       K_THREAD_STACK_SIZEOF(realtime_metrics_workq_stack), realtime_metrics_priority - 1, NULL);
    ...
    realtime_metrics_tid = k_thread_create(
        &realtime_metrics_thread_data, realtime_metrics_stack_area, K_THREAD_STACK_SIZEOF(realtime_metrics_stack_area),
        realtime_metrics_thread_fn, NULL, NULL, NULL, realtime_metrics_priority, 0, K_NO_WAIT);
        ...
        if (ret == 0)
        {
            ...
            switch (msg.type)
            {
                ...
                case MSG_TYPE_REALTIME_METRICS:
                    // DIRECT PATH: Use bilateral metrics exactly as received
                    LOG_INF("Received bilateral metrics: Cadence=%u, Pace=%u, GCT=%u",
...
static void calculate_realtime_metrics(void)
{
    ...
    if (has_bhi360)
    {
        ...
        cadence_tracker_update(&metrics_state.cadence_tracker, data->step_count, metrics_state.current_time_ms);
    ...
    else
    {
        ...
        bool left_step =
            (data->left_contact_duration_ms > 100 && data->left_contact_duration_ms > last_left_contact_duration);
        bool right_step =
            (data->right_contact_duration_ms > 100 && data->right_contact_duration_ms > last_right_contact_duration);
            ...
            if (last_step_time_ms > 0)
            {
                ...
                { // 30-300 spm range
                    ...
                    if (count > 0)
                    {
                        ...
                        LOG_DBG("Foot-based cadence: %.1f spm (interval: %u ms)", (double)current_cadence,
                                avg_interval);
                    }
                }
            }
        ...
        cadence_tracker_update(&metrics_state.cadence_tracker, step_count_fallback, metrics_state.current_time_ms);
    }

    // Update pace estimator with GPS if available
    if (metrics_state.last_gps_timestamp > 0 &&
        (metrics_state.current_time_ms - metrics_state.last_gps_timestamp) < 5000)
    {
        pace_estimator_update_with_gps(&metrics_state.pace_estimator, metrics_state.latest_gps_speed_cms,
                                       data->delta_time_ms);
    }
    else
    {
    ...
    if (data->left_contact_duration_ms > 0)
    {
        ...
        metrics_state.last_contact_times[metrics_state.variance_index] = (float)data->left_contact_duration_ms;
    ...
    metrics_state.current_metrics.stride_duration_ms =
        (metrics_state.left_stride_duration + metrics_state.right_stride_duration) / 2;

    // Stride duration asymmetry
    metrics_state.current_metrics.stride_duration_asymmetry =
        calculate_asymmetry_percentage(metrics_state.left_stride_duration, metrics_state.right_stride_duration);
    ...
    if (!has_bhi360 || speed_ms == 0)
    {
        ...
        if (data->left_contact_duration_ms > 0 || data->right_contact_duration_ms > 0)
        {
            ...
            if (avg_gct > 0)
            {
                ...
                if (estimated_stride < 0.5f)
                    estimated_stride = 0.5f;
                if (estimated_stride > 2.0f)
                    estimated_stride = 2.0f;

                metrics_state.left_stride_length = estimated_stride;
                metrics_state.right_stride_length = estimated_stride;

                LOG_DBG("Estimated stride from GCT: %.2f m (GCT: %u ms)", estimated_stride, avg_gct);
            }
        }
    }
    else
    ...
    metrics_state.current_metrics.stride_length_asymmetry = calculate_asymmetry_percentage(
        (uint16_t)(metrics_state.left_stride_length * 100), (uint16_t)(metrics_state.right_stride_length * 100));
    ...
    if (metrics_state.current_metrics.stride_length_cm > 0)
    {
        vertical_ratio = ((float)metrics_state.current_metrics.vertical_oscillation_cm /
                          (float)metrics_state.current_metrics.stride_length_cm) *
                         100.0f;
    }
    ...
    form_score_calculate(&metrics_state.form_score, avg_left_contact, balance, variability, avg_pronation);
    ...
    if (estimated_pace == 0 && current_cadence > 0)
    {
        ...
        if (stride_length_m == 0)
            stride_length_m = 1.2f; // Default 1.2m
        float speed_mps = (current_cadence / 60.0f) * stride_length_m;
        if (speed_mps > 0)
        {
            estimated_pace = (uint16_t)(1000.0f / speed_mps);
            LOG_DBG("Pace from cadence fallback: %d s/km (cadence: %.1f, stride: %.2fm)", estimated_pace,
                    current_cadence, stride_length_m);
        }
    }
    metrics_state.current_metrics.pace_sec_km = estimated_pace;
    metrics_state.current_metrics.distance_m =
        metrics_state.pace_estimator.distance_m; // No cast needed, both are uint32_t now
    ...
    // Strike patterns
    metrics_state.current_metrics.left_strike_pattern =
        data->left_contact_phase == 1 ? 0 : (data->left_fore_pct > 50 ? 2 : 1);
    metrics_state.current_metrics.right_strike_pattern =
        data->right_contact_phase == 1 ? 0 : (data->right_fore_pct > 50 ? 2 : 1);
    ...
    if (metrics_state.metrics_count % 50 == 0)
    {
        LOG_INF("Metrics: Cadence=%d spm, Pace=%d s/km, Form=%d%%, Balance=%d%%",
                metrics_state.current_metrics.cadence_spm, metrics_state.current_metrics.pace_sec_km,
                metrics_state.current_metrics.form_score, balance);
    ...
    memcpy(&out_msg.data.realtime_metrics, &metrics_state.current_metrics, sizeof(realtime_metrics_t));
            ...
            if (last_step_time_ms > 0)
            {
                ...
                { // 30-300 spm range
                    ...
                    if (count > 0)
                    {
                        ...
                        LOG_DBG("Secondary foot-based cadence: %.1f spm (interval: %u ms)", (double)current_cadence,
                                avg_interval);
                    }
                }
            }
    ...
    if (metrics_state.metrics_count % 50 == 0)
    {
        LOG_INF("Secondary Metrics: Cadence=%d spm, GCT=%d ms, Pace=%d s/km", metrics_state.current_metrics.cadence_spm,
                metrics_state.current_metrics.ground_contact_ms, metrics_state.current_metrics.pace_sec_km);
...
static void send_ble_update(void)
{
    // USE BILATERAL METRICS DIRECTLY - NO CORRUPTING PRESERVATION LOGIC
    realtime_metrics_t metrics_to_send;

    // Copy the current metrics exactly as they are from gait events
    ...
    memcpy(&ble_msg.data, &metrics_to_send,
           sizeof(metrics_to_send) > sizeof(ble_msg.data) ? sizeof(ble_msg.data) : sizeof(metrics_to_send));
    ...
    if (ble_phone_is_connected())
    {
        ...
        {
            LOG_DBG("Metrics sent to Bluetooth");
        }
    }

    // Send to Data module (primary device only)
#if IS_ENABLED(CONFIG_PRIMARY_DEVICE)
    ...
    memcpy(&data_msg.data, &metrics_to_send,
           sizeof(metrics_to_send) > sizeof(data_msg.data) ? sizeof(data_msg.data) : sizeof(metrics_to_send));
    ...
    {
        LOG_DBG("Metrics sent to Data module");
    }
#endif

    // Log alerts if any (but don't modify the metrics!)
