...
} weight_cal = {.scale_factor = 0.0168f, // ~100N max per sensor (16 sensors total)
 ...
 *       Data Requirements: Motion sensor data (BHI360 for cadence), Foot sensor data (both feet for contact and
 * strike).
 */
...
static void activity_metrics_init(void)
{
    ...
    k_work_queue_start(&activity_metrics_work_q, activity_metrics_workq_stack,
                       K_THREAD_STACK_SIZEOF(activity_metrics_workq_stack), activity_metrics_priority - 1, NULL);
    ...
    step_detection_register_callback([](const StepEvent *event) {
        ...
        LOG_WRN("Step event: foot=%d, contact=%.1fms, flight=%.1fms, strike=%d", event->foot,
                (double)event->contact_time_ms, (double)event->flight_time_ms, event->strike_pattern);
    });
    ...
    activity_metrics_tid = k_thread_create(
        &activity_metrics_thread_data, activity_metrics_stack_area, K_THREAD_STACK_SIZEOF(activity_metrics_stack_area),
        activity_metrics_thread_fn, NULL, NULL, NULL, activity_metrics_priority, 0, K_NO_WAIT);
        ...
        if (ret == 0)
        {
            ...
            switch (msg.type)
            {
                ...
                case MSG_TYPE_SAVE_WEIGHT_CALIBRATION:
                    ...
                    LOG_INF("Weight calibration updated: scale=%.6f, offset=%.6f", (double)weight_cal.scale_factor,
                            (double)weight_cal.nonlinear_a);
                    break;
                ...
                case MSG_TYPE_WEIGHT_CALIBRATION_DATA:
                    ...
                    {
                        LOG_INF("Weight calibration loaded: scale=%.6f", (double)weight_cal.scale_factor);
                    }
                    else
                    {
                ...
                default:
                    LOG_WRN("Received unsupported message type %d from %s", msg.type, get_sender_name(msg.sender));
                    break;
            }
        }
    }
...
static void process_command_work_handler(struct k_work *work)
{
    ...
    if (strcmp(work_item->command, "START_ACTIVITY") == 0)
    {
        ...
        SessionHeader header = {
            ...
            .user_weight_kg =
                (uint16_t)(sensor_data.weight_measurement_valid ? (sensor_data.calculated_weight_kg * 10) : 700),
            .user_height_cm = 175,
            .user_age = 30,
            .user_gender = 0,
...
static void weight_calibration_work_handler(struct k_work *work)
{
    ...
    while (weight_measurement.measurement_in_progress)
    {
        ...
        if (now - sensor_data.last_left_pressure_update > 200 || now - sensor_data.last_right_pressure_update > 200)
        {
            data_valid = false;
        }
...
static void calculate_realtime_metrics(void)
{
    ...
    if (!session_state.baseline_established && sensor_data.avg_contact_time > 0)
    {
        ...
        if (valid_samples >= 5)
        {
            ...
            LOG_INF("Baseline established: contact=%.1fms, form=%f", (double)session_state.baseline_contact_time,
...
static uint8_t calculate_form_score(void)
{
    ...
    if (left_state.initial_contact_region == STRIKE_HEEL || right_state.initial_contact_region == STRIKE_HEEL)
    {
        score -= 5;
    }
...
static uint8_t calculate_efficiency_score(float contact_time, float flight_time)
{
    if (contact_time <= 0 || flight_time <= 0)
        return 50;
...
static uint8_t calculate_fatigue_level(void)
{
    ...
    float contact_increase =
        (sensor_data.avg_contact_time - session_state.baseline_contact_time) / session_state.baseline_contact_time;
...
void activity_session_start(const SessionHeader *header)
{
    ...
    LOG_INF("Activity session started: id=%u, type=%d, subtype=%d", header->session_id, header->activity_type,
            header->activity_subtype);
}
...
void activity_session_stop(void)
{
    ...
    LOG_INF("Activity session stopped: duration=%us, steps=%u, distance=%um", duration_ms / 1000, total_steps,
            distance_m);
    ...
    {
        LOG_INF("Session summary sent to Bluetooth: distance=%dm, avg_pace=%ds/km, avg_cadence=%dspm", distance_m,
                summary_msg.data.session_summary.avg_pace_sec_km, summary_msg.data.session_summary.avg_cadence_spm);
    }
}
...
static uint16_t calculate_current_pace(void)
{
    ...
    if (session_state.last_gps_update_time > 0 && (current_time - session_state.last_gps_update_time) < 30000)
    {
        // Use GPS-based pace
...
void activity_session_process_gps_update(const GPSUpdateCommand *gps_data)
{
    ...
    LOG_INF("GPS update: lat=%d, lon=%d, speed=%u cm/s, dist=%u m, acc=%u m", gps_data->latitude_e7,
    ...
    if (gps_data->distance_m > 0)
    {
        ...
        uint32_t sensor_distance_cm = session_state.total_distance_cm - session_state.distance_at_last_gps;
        float sensor_distance_m = sensor_distance_cm / 100.0f;

        // Calculate correction factor
        if (sensor_distance_m > 10.0f)
        { // Only calibrate if we've moved at least 10m
            ...
            if (correction > 0.8f && correction < 1.2f)
            {
                ...
                LOG_INF("Stride calibration: GPS=%um, sensor=%.1fm, correction=%.3f", gps_data->distance_m,
                        (double)sensor_distance_m, (double)session_state.stride_correction_factor);
            }
            else
            {
        ...
        session_state.total_distance_cm = session_state.distance_at_last_gps + (gps_data->distance_m * 100);
    ...
    if (gps_data->elevation_change_m != 0)
    {
        ...
        LOG_WRN("Elevation change: %dm, total gain: %dcm", gps_data->elevation_change_m,
                session_state.total_elevation_gain_cm);
    }
}
...
static uint32_t calculate_total_distance(void)
{
    ...
    if (session_state.stride_correction_factor > 0.8f && session_state.stride_correction_factor < 1.2f &&
        session_state.last_gps_update_time > 0)
    {
...
static void update_splits()
{
    if (session_state.split_distance_cm == 0)
        return;
...
static void process_weight_measurement(void)
{
    ...
    if (data_valid)
    {
            ...
            weight_measurement.buffer_count >= 10)
        {
            ...
            if (raw_weight_kg > 20.0f && raw_weight_kg < 300.0f)
            { // Sanity check
                ...
                LOG_INF("Weight measurement complete: raw=%.1f kg, calibrated=%.1f kg", (double)raw_weight_kg,
                        (double)calibrated_weight_kg);
...
static float calculate_weight_from_pressure(void)
{
    ...
    if (weight_cal.nonlinear_a != 1.0f || weight_cal.nonlinear_b != 0.0f || weight_cal.nonlinear_c != 0.0f)
    {
        ...
        total_force_n = weight_cal.nonlinear_a * calibrated_adc * calibrated_adc +
                        weight_cal.nonlinear_b * calibrated_adc + weight_cal.nonlinear_c;
    }
    else
    {
...
static float apply_weight_calibration(float raw_weight_kg)
{
    ...
    // Apply sanity limits
    if (calibrated_weight < 0.0f)
        calibrated_weight = 0.0f;
    if (calibrated_weight > 500.0f)
        calibrated_weight = 500.0f; // Max reasonable weight
...
static bool is_person_standing_still(void)
{
    ...
    float acc_magnitude = sqrtf(sensor_data.lacc_x * sensor_data.lacc_x + sensor_data.lacc_y * sensor_data.lacc_y +
                                sensor_data.lacc_z * sensor_data.lacc_z);

    // Check gyroscope magnitude
    float gyro_magnitude = sqrtf(sensor_data.gyro_x * sensor_data.gyro_x + sensor_data.gyro_y * sensor_data.gyro_y +
...
static void perform_weight_calibration(void)
{
    ...
    {
        LOG_ERR("Invalid calibration: ADC reading (%.2f) is not positive", (double)avg_pressure);
...
static bool app_event_handler(const struct app_event_header *aeh)
{
    ...
    if (is_motion_sensor_stop_activity_event(aeh) || is_foot_sensor_stop_activity_event(aeh))
    {
