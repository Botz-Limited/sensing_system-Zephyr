...
static bool detect_swing_phase_imu(const imu_data_t *imu, bool was_in_swing)
{
    ...
    float gyro_magnitude = sqrtf(imu->gyro_x * imu->gyro_x + imu->gyro_y * imu->gyro_y + imu->gyro_z * imu->gyro_z);

    float accel_magnitude =
        sqrtf(imu->accel_x * imu->accel_x + imu->accel_y * imu->accel_y + imu->accel_z * imu->accel_z);
    ...
    bool in_swing = (gyro_magnitude < SWING_GYRO_THRESHOLD) && (accel_magnitude > SWING_ACCEL_MIN) &&
...
static void detect_turn_event(gait_event_detector_t *detector, const imu_data_t *imu, float dt)
{
    ...
    else if (in_turn)
    {
        ...
        if (fabsf(yaw_angle) > TURN_ANGLE_THRESHOLD)
        {
            ...
            LOG_INF("Turn completed: %.1f degrees in %.1f ms", (double)turn_angle_deg, (double)turn_duration);
        }
        in_turn = false;
    }
}
...
static void detect_step_from_imu(gait_event_detector_t *detector, const imu_data_t *imu)
{
    ...
    if (current_time - last_step_time > 300)
    { // Min 300ms between steps
        float avg = 0.0f;
        for (int i = 0; i < 10; i++)
        {
            avg += accel_history[i];
        ...
        if (accel_mag > avg * 1.2f && accel_mag > 11.0f)
        {
            ...
            LOG_DBG("IMU step detected at time %u ms (total: %u)", current_time, detector->total_step_count);
        }
    }
}
 ...
 */
static void update_velocity_integration(gait_event_detector_t *detector, const imu_data_t *imu_data, float dt)
{
    ...
    if (vertical_position > max_height)
        max_height = vertical_position;
    if (vertical_position < min_height)
...
static bool detect_mid_stance(const foot_samples_t *foot_data, float total_force, float last_total_force)
{
    ...
    uint16_t mid_force =
        foot_data->values[2] + foot_data->values[3] + foot_data->values[4] + foot_data->values[5]; // Mid sensors
 ...
 */
static float refine_initial_contact(const foot_samples_t *buffer, int ic_index, int buffer_size,
                                    uint8_t *strike_pattern_out)
{
    ...
    {
        uint16_t delta =
            (ic_sample->values[i] > prev_sample->values[i]) ? (ic_sample->values[i] - prev_sample->values[i]) : 0;
        if (delta > max_delta)
        {
            max_delta = delta;
...
static gait_phase_t detect_enhanced_gait_phase(gait_event_detector_t *detector, int current_index)
{
    ...
    const foot_samples_t *prev_foot_data = (current_index > 0) ? &detector->foot_buffer[current_index - 1] : foot_data;
...
static void detect_ic_to_events(gait_event_detector_t *detector)
{
    ...
    if (detector->buffer_count < detector->sampling_rate)
    ...
    if (detector->use_advanced_features)
    {
        ...
        for (int i = 0; i < MIN(10, samples_to_process); i++)
        {
            ...
            if (imu->accel_x != 0 || imu->accel_y != 0 || imu->accel_z != 0 || imu->gyro_x != 0 || imu->gyro_y != 0 ||
                imu->gyro_z != 0)
            {
                has_imu_data = true;
                break;
        ...
        if (has_imu_data)
        {
            build_motion_mask(&detector->motion_mask, detector->imu_buffer, samples_to_process,
        ...
        {
            int idx = (start_idx + i) % GAIT_BUFFER_SIZE_SAMPLES;
            detector->pressure_signal_filled[i] = (float)calculate_total_pressure(&detector->foot_buffer[idx]);
        }

        int gaps = kalman_fill_gaps(detector->pressure_signal_filled, samples_to_process,
                                    50.0f,                /* threshold for gap detection */
                                    (SAMPLE_RATE_HZ / 5), /* max gap = 400ms @ 200Hz */
    ...
    float adaptive_threshold = calculate_adaptive_threshold(detector->foot_buffer, start_idx, samples_to_process);

    /* Log threshold for debugging */
    static int threshold_log_counter = 0;
    if (++threshold_log_counter % 10 == 0)
    { // Log every 10th call
        LOG_INF("Adaptive threshold: %.1f (processing %d samples)", adaptive_threshold, samples_to_process);
    }
    ...
    for (int i = 1; i < samples_to_process && detector->ic_count < GAIT_MAX_EVENTS_PER_CHUNK; i++)
    {
        ...
        if (!prev_contact && curr_contact)
        {
            ...
            float refined_ic_time =
                refine_initial_contact(detector->foot_buffer, curr_idx, GAIT_BUFFER_SIZE_SAMPLES, &strike_pattern);
            ...
            LOG_DBG("IC detected at index %d (refined: %.1f), pressure %.1f > threshold %.1f, strike=%d", i,
                    refined_ic_time, curr_pressure, adaptive_threshold, strike_pattern);
        }
    ...
    if (detector->use_advanced_features)
    {
        LOG_DBG("Detected %d IC and %d TO events (threshold: %.1f, gaps_filled: %lu, rejected: %lu)",
                detector->ic_count, detector->to_count, adaptive_threshold, (unsigned long)detector->gaps_filled,
                (unsigned long)detector->false_positives_rejected);
    }
    else
    {
        LOG_DBG("Detected %d IC and %d TO events (threshold: %.1f)", detector->ic_count, detector->to_count,
                adaptive_threshold);
    }
}
...
static void create_stride_segments(gait_event_detector_t *detector)
{
    ...
    for (int i = 0; i < detector->ic_count - 1 && detector->stride_count < GAIT_MAX_EVENTS_PER_CHUNK; i++)
    {
        ...
        LOG_DBG("Stride segment %d: IC=%d (%.2fs), TO=%d (%.2fs), next_IC=%d (%.2fs)", detector->stride_count - 1, ic1,
                (double)segment->ic_timestamp, to_idx, (double)segment->to_timestamp, ic2,
                (double)segment->next_ic_timestamp);
    }
    ...
    for (int s = 0; s < detector->stride_count && detector->metrics_count < GAIT_MAX_EVENTS_PER_CHUNK; s++)
    {
        ...
        for (int i = segment->ic_index; i <= segment->to_index; i++)
        {
            ...
            if (current_phase == PHASE_LOADING && i > segment->ic_index)
            {
                ...
                if (prev_force > 0)
                {
                    float dt = detector->timestamps[buf_idx] -
                               detector->timestamps[(start_idx + i - 1) % GAIT_BUFFER_SIZE_SAMPLES];
                    if (dt > 0 && dt < 0.1f)
                    {
                        uint16_t loading_rate =
                            calculate_loading_rate(current_force, prev_force, (uint16_t)(dt * 1000));
        ...
        for (int i = segment->ic_index; i < segment->next_ic_index; i++)
        {
            ...
            if (dt > 0 && dt < 0.1f)
            {
                ...
                if (fabsf(detector->velocities[buf_idx].x) > 0.001f ||
                    fabsf(detector->velocities[buf_idx].y) > 0.001f || fabsf(detector->velocities[buf_idx].z) > 0.001f)
                {
        ...
        if (!has_valid_velocity || fabsf(stride_length) < 0.1f || fabsf(stride_length) > 5.0f)
        {
            ...
            if (m->cadence > 0)
            {
                ...
                LOG_DBG("Estimated stride length: %.2fm from cadence %.1f spm and height %.2fm", stride_length,
                        m->cadence, height);
            }
            else
            {
        ...
        if (detector->use_advanced_features)
        {
            ...
            m->speed_anthropometric =
                anthro_calculate_speed(&detector->anthro_model, m->cadence, m->stride_length, m->gct, swing_time);
        }
        else
        {
        ...
        if (detector->use_advanced_features)
        {
            LOG_WRN("ADVANCED Stride %d metrics: GCT=%.3fs, duration=%.3fs, cadence=%.1f, length=%.2fm, speed=%.2fm/s "
                    "(smoothed)",
                    s, m->gct, m->duration, m->cadence, m->stride_length, m->speed_anthropometric);
        }
        else
        {
            LOG_WRN("NOT ADVANCED Stride %d metrics: GCT=%.3fs, duration=%.3fs, cadence=%.1f, length=%.2fm", s, m->gct,
                    m->duration, m->cadence, m->stride_length);
        }
    }
...
int gait_events_process(gait_event_detector_t *detector)
{
    ...
    if (detector->stride_count > 0)
    {
        ...
        if (samples_to_skip > 0 && samples_to_skip < detector->buffer_count)
        {
            ...
            LOG_DBG("Cleared %d samples after processing, %d remain in buffer (overlap: %d)", samples_to_skip,
                    detector->buffer_count, overlap_samples);
 ...
 */
int gait_events_get_metrics(gait_event_detector_t *detector, gait_metrics_t *metrics_out, int max_metrics)
{
