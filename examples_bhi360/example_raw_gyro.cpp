// Example: Configure and read BHI360 Raw Gyroscope at maximum rate (800 Hz)
...
LOG_MODULE_REGISTER(example_bhi360_gyro, LOG_LEVEL_INF);

#define BHI360_NODE DT_NODELABEL(bhi360)
static const struct device *const bhi360_dev = DEVICE_DT_GET(BHI360_NODE);

#define GYRO_SENSOR_ID BHY2_SENSOR_ID_GYRO_PASS

static struct bhy2_dev bhy2;

static void gyro_callback(const struct bhy2_fifo_parse_data_info *info, void *ref)
{
    struct bhy2_data_xyz data;
    bhy2_parse_xyz(info->data_ptr, &data);
    LOG_INF("Raw Gyro: x=%.2f, y=%.2f, z=%.2f",
...
void main(void)
{
    ...
    rslt = bhy2_set_virt_sensor_cfg(GYRO_SENSOR_ID, sample_rate, report_latency_ms, &bhy2);
    if (rslt != BHY2_OK) {
        LOG_ERR("Failed to set GYRO config");
        return;
    }
    rslt = bhy2_register_fifo_parse_callback(GYRO_SENSOR_ID, gyro_callback, NULL, &bhy2);
    if (rslt != BHY2_OK) {
        LOG_ERR("Failed to register GYRO callback");
        return;
    }
    LOG_INF("BHI360 Raw Gyroscope configured at 800 Hz");
    uint8_t fifo[512];
    while (1) {
